...
y.cnt = 1000000
x.p = .495
ruin.sim = replicate(n.reps, gen.ruin(n = 10000, x.cnt = x.cnt, y.cnt = y.cnt, x.p = x.p))
ruin.sim[ruin.sim == 0] = NA
hist(apply(ruin.sim == x.cnt + y.cnt | is.na(ruin.sim), 2, which.max), nclass = 100, col = '8', main = "Distribution of Number of Turns",
     xlab = "Turn Number")
abline(v = mean(apply(ruin.sim == x.cnt + y.cnt | is.na(ruin.sim), 2, which.max)), lwd = 3, col = 'red')
abline(v = median(apply(ruin.sim == x.cnt + y.cnt | is.na(ruin.sim), 2, which.max)), lwd = 3, col = 'green')
x.annihilation = apply(ruin.sim == x.cnt + y.cnt, 2, which.max)
...
points(x.cnt + y.cnt - mean.state, pch = 16, cex = .5, col = 'blue')
lines(x.cnt + y.cnt - mean.state, col = 'blue')